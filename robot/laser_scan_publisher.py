...
class LaserScanNode(object):
    def __init__(self):
        ...
        self.frames = ['/laser_front', '/laser_front_right', '/laser_front_left', '/laser_right', '/laser_left']

        self.tf_listener = tf.TransformListener()
        for i in xrange(len(self.frames)):
            self.tf_listener.waitForTransform('/map', self.frames[i], rospy.Time(0), rospy.Duration(10.))

        while not rospy.is_shutdown():
            laser_positions, obstacle_positions = self.read_obstacle_positions()
            self.publish_scans(laser_positions, obstacle_positions)

    def read_obstacle_positions(self):
        laser_positions = -1. * np.ones((len(self.frames), 2))
        laser_obstacle_positions = -1. * np.ones((len(self.frames), self.number_of_readings, 2))

        for i in xrange(len(self.frames)):
            try:
                (translation, quat_rotation) = self.tf_listener.lookupTransform('/map', self.frames[i], rospy.Time(0))
            except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
                return laser_positions, laser_obstacle_positions

            laser_positions[i,0] = translation[0]
            laser_positions[i,1] = translation[1]
            euler_rotation = tf.transformations.euler_from_quaternion(quat_rotation)

            angle = euler_rotation[2] - self.scanner_min_angle
            for j in xrange(self.number_of_readings):
                ...
                try:
                    ...
                    result = proxy(laser_positions[i,0], laser_positions[i,1], direction_x, direction_y)
                    if result.success == 'success':
                        laser_obstacle_positions[i,j,0] = result.position_x
                        laser_obstacle_positions[i,j,1] = result.position_y
                    else:
                        laser_obstacle_positions[i,j,0] = self.scanner_max_range
                        laser_obstacle_positions[i,j,1] = self.scanner_max_range
    ...
    def publish_scans(self, laser_positions, obstacle_positions):
        for i in xrange(len(self.frames)):
            ranges = []
            for j in xrange(self.number_of_readings):
                distance = self.distance(laser_positions[i][0], laser_positions[i][1], obstacle_positions[i][j][0], obstacle_positions[i][j][1])
                ranges.append(distance)

            laser_scan = LaserScan()
            laser_scan.header.stamp = rospy.Time.now()
            laser_scan.header.frame_id = self.frames[i]
