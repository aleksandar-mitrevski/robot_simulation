...
class MotionModel(object):
    ...
    def sample_motion_model(self, pose, motion_command):
        ...
        delta_trans_prime = delta_trans + self.sample_normal_noise(self.noise_params.alpha3 * self.delta_trans * self.delta_trans + self.noise_params.alpha4 * delta_rot1 * delta_rot1 + self.noise_params.alpha4 * delta_rot2 * delta_rot2)
        delta_rot2_prime = delta_rot2 + self.sample_normal_noise(self.noise_params.alpha1 * delta_rot2 * delta_rot + self.noise_params.alpha2 * delta_trans * delta_trans)
